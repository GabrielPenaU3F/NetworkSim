...
def test_destination_added_to_table(routing_table):
    ...
    routing_table.add_entry(node, iface)
    assert routing_table.get_interface(node) == iface

def test_unreachable_node_is_rejected():
    node_a = DummyNode(address='192.168.0.1')
    node_b = DummyNode(address='192.168.0.2')
    node_c = DummyNode(address='192.168.0.3')
    iface = DummyInterface()
    routing_table = RoutingTable(node_a)
    routing_table.add_entry(node_b, iface)

    err_msg = 'Node 192.168.0.3 is unreachable from 192.168.0.1'
    assert routing_table.get_interface(node_b) == iface
    with pytest.raises(NetworkError, match=err_msg):
        routing_table.get_interface(node_c)
