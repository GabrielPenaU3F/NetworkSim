...
def test_add_node(empty_graph, three_dummy_hosts):
    a, _, _ = three_dummy_hosts
    empty_graph.add_node(a)
    assert empty_graph.node_count() == 1

def test_add_duplicate_node_does_not_increase_count(empty_graph, three_dummy_hosts):
    a, _, _ = three_dummy_hosts
    empty_graph.add_node(a)
    empty_graph.add_node(a)
    assert empty_graph.node_count() == 1

def test_get_edge_to_returns_correct_edge(empty_graph, three_dummy_hosts):
...
def test_get_edge_to_returns_none_if_not_connected(empty_graph, three_dummy_hosts):
...
def test_add_edge_registers_both_directions(empty_graph, three_dummy_hosts):
    a, b, _ = three_dummy_hosts
    empty_graph.add_node(a)
    empty_graph.add_node(b)
    empty_graph.add_edge(a, b)
...
def test_get_neighbors_of_isolated_node(empty_graph, three_dummy_hosts):
    a, _, _ = three_dummy_hosts
    empty_graph.add_node(a)
    assert empty_graph.get_neighbors(a) == []

def test_get_neighbors_of_connected_node(example_graph, three_dummy_hosts):
    a, b, c = three_dummy_hosts
    neighbors = example_graph.get_neighbors(b)
    assert a in neighbors
    assert c in neighbors

def test_cannot_create_parallel_edges(empty_graph, three_dummy_hosts):
    a, b, _ = three_dummy_hosts
    empty_graph.add_node(a)
    empty_graph.add_node(b)
...
def test_cannot_create_antiparallel_edges(empty_graph, three_dummy_hosts):
    a, b, _ = three_dummy_hosts
    empty_graph.add_node(a)
    empty_graph.add_node(b)
    with pytest.raises(ValueError, match='An edge between these nodes already exists'):
