...
class CommSystem:
    ...
    def build_physical_layer(self, config):
        ...
        channel = BinarySymmetricChannel(channel_code, error_prob)
        return channel

    def build_link_layer(self, config):
        link_configs = config.get_link_configs()
        block_size = link_configs.get('block_size')
        max_retries = link_configs.get('max_retries')
        link = Link(self.channel, block_size, max_retries)
        return link
    ...
    def transmit(self, codebook, message):
        ...
        received_bits = self.channel.transmit(codebook, source_bits)
